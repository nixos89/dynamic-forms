...
} from "../redux/store/formElementTypes";
...
function FormComponent(props) {
	...
	const deleteButtonFunc = (formId, id) => {
		return (
			...
			>
				<span style={{ color: "white" }} role="img" aria-label="x-sign">
					❌
				</span>
			</button>
		);
	...
	return (
		<div key={key} className="justify-content-center">
					...
					{im_formElements.map((formElement, index) => {
						...
						switch (im_formElementType) {
							...
							default: {
								return (
									<p>
										Ingen inntastingsfelt lagt til i gjeldende skjemaer ennå!
									</p>
								);
							}
						}
			...
			<div className="form-group row justify-content-center">
				<button onClick={shareForm} className="btn btn-outline-success btn-sm">
					Del skjema
				</button>
				&nbsp;
				<button
					onClick={() => deleteForm(im_formId)}
...
function mapStateToProps(state, ownProps) {
	...
	const updatedFormElements = mainReducer.getIn([
		"forms",
		formIndex,
		"formElements",
	]);
